...
class TestRecipe:
    ...
    def test_has_attributes(self):
        ...
        with app.app_context():
            ...
            # Create test user
            user = User(username="chef123", _password_hash=generate_password_hash("secret"))
            db.session.add(user)
            db.session.commit()
    ...
    def test_requires_title(self):
        ...
        with app.app_context():
            ...
            user = User(username="testuser", _password_hash=generate_password_hash("secret"))
            db.session.add(user)
            db.session.commit()

            recipe = Recipe(
                instructions="This is a long enough instruction to meet the requirement.",
    ...
    def test_requires_50_plus_char_instructions(self):
        '''must raise either a sqlalchemy.exc.IntegrityError or custom ValueError for short instructions'''
        ...
        with app.app_context():
            ...
            user = User(username="shortinst", _password_hash=generate_password_hash("secret"))
            db.session.add(user)
            db.session.commit()
